...
server.post("/contributions", (req, res) => {
	...
	const id = contributions.length
		? contributions[contributions.length - 1].id + 1
		: 1;
	...
	const storyContributions =
		contributions.filter((c) => c.storyId === storyId).length + 1; // Include new contribution
